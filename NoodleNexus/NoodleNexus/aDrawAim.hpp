...
public:
	...
	void DrawRay2(glm::vec3 pos, glm::vec3 posEnd, GLuint program)
	{
		...
		while (glm::distance(pos, movingPoint) < distance)
		{
			...
			DrawDebugSphere(movingPoint, glm::vec4(0.0f, 0.0f, 1.0f, 1.0f), 0.01f, program);
		}
	...
	void UpdateAim()
	{
		...
		if (object->scene->physicsManager->RayCast(startPos, endPos, collisions, false))
		{
			...
			object->scene->DrawSphere(collisions[0].vecTriangles[0].intersectionPoint , glm::vec4(1.f, 0.f, 0.f, 1.f),1.f, program);
